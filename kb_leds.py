...
for dev in watched_devices:
 if watched_devices[dev]['name'].startswith('storage'):
  ...
  if watched_devices[dev]['name'].endswith('tx'):
   watched_devices[dev]['gen'] = get_network_activity('wl' if watched_devices[dev]['name'].startswith('w') else 'en', 'tx')
  else:
   watched_devices[dev]['gen'] = get_network_activity('wl' if watched_devices[dev]['name'].startswith('w') else 'en', 'rx')
...
for dev in watched_devices:
 ...
 watched_devices[dev]['threshold'] = int(watched_devices[dev]['name'].split('=')[1]) if watched_devices[dev]['name'].startswith('cpu') else 0
